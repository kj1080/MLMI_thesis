...
from moe_bert_final_layer import BertWithMoE
...
# ===== Options =====
use_moe = True  # <-- Toggle this to enable MoE BERT

pause_encoding = True
question_first = "late_fusion" # 'late_fusion_different_prompt'
majority_voting = False
model_used = "bert"  # "bert", "roberta", "ensemble" or "gpt4"
...
def run_pbft(seed, train_df, val_df, test_df):
    ...
    for plm_type, model_path in model_configs:
        for template_text in templates:
            ...
            if use_moe and plm_type == "bert":
                model = BertWithMoE(model_name=model_path, num_experts=4)
                from transformers import BertTokenizer
                tokenizer = BertTokenizer.from_pretrained(model_path)
                from openprompt.plms.mlm import MLMTokenizerWrapper
                WrapperClass = MLMTokenizerWrapper
                plm = None  # just to avoid undefined later
            else:
            ...
            verbalizer = ManualVerbalizer(
                ...
                label_words={"alzheimer": ["dementia", "alzheimer's"], "healthy": ["normal", "healthy"]},
                tokenizer=tokenizer
            )
            ...
            if use_moe and plm_type == "bert":
                model = model  # already defined above
            else:
            ...
            for epoch in tqdm(range(num_epochs), desc=f"Epochs ({model_path})", position=1, leave=False):
                ...
                for i, batch in enumerate(train_loader):
                    ...
                    # Remove 'label' before passing to model
                    batch_inputs = {k: v for k, v in batch.items() if k != 'label'}
                    logits = model(**batch_inputs)[0] if use_moe and plm_type == "bert" else model(batch_inputs)

                    
                    _, gates = model(**batch_inputs)
                    avg_gate_q = gates['query'].mean(dim=(0, 1)).cpu().detach().numpy()
                    wandb.log({f"moe_expert_usage_query_{i}": g for i, g in enumerate(avg_gate_q)})
