...
load_dotenv(dotenv_path="secure.env")  # or just load_dotenv() if it's in the same dir
...
model_used = "ensemble"  # "bert", "roberta", "ensemble" or "gpt4"
...
def gpt4_classify(text):
    prompt = f"""
You are a clinical speech assessment expert diagnosing Alzheimer's Disease from verbal picture description tasks.

Here is a patient response:

"{text}"

Classify this response as either:
- Alzheimer's
- Healthy

Your response must be exactly one word: Alzheimer's or Healthy.
"""
    try:
        response = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are an expert clinical speech analyst."},
                {"role": "user", "content": prompt}
            ],
            temperature=0
        )
        answer = response["choices"][0]["message"]["content"].strip().lower()
        return 1 if "alzheimer" in answer else 0
    except Exception as e:
        print("Error during GPT-4 classification:", e)
...
def run_pbft(seed, train_df, val_df, test_df):
    if model_used == "gpt4":
        print("Running GPT-4 inference on test set...")
        gpt_preds = []
        gpt_labels = []

        for _, row in test_df.iterrows():
            pred = gpt4_classify(row["text"])
            gpt_preds.append(pred)
            gpt_labels.append(row["label"])

        gpt_acc = accuracy_score(gpt_labels, gpt_preds)
        print(f"GPT-4 Test Accuracy: {gpt_acc:.4f}")
        wandb.log({"gpt4_test_accuracy": gpt_acc})
        return gpt_acc
    ...
    if question_first == "late_fusion":
        ...
        # templates = ["This individual {'mask'}. Based on the individual's speech patterns in this transcript: {'placeholder':'text_a'}.", "Based on the individual's speech patterns in this transcript: {'placeholder':'text_a;'}. This individual {'mask'}."]
        templates = ["This individual {'mask'}. We're basing this diagnosis on the individual's speech patterns in this transcript: {'placeholder':'text_a'}.", 
             "We're basing this diagnosis on the individual's speech patterns in this transcript:: {'placeholder':'text_a'}. This individual {'mask'}."]
    ...
    for plm_type, model_path in model_configs:
        for template_text in templates:
            ...
            verbalizer = ManualVerbalizer(
                ...
                # label_words={"alzheimer": ["has dementia", "has alzheimer's"],
                #              "healthy": ["is normal", "is healthy"]},
                label_words={"alzheimer": ["has alzheimer's"],
                             "healthy": ["is healthy"]},
                tokenizer=tokenizer
            )
