...
# =========== Path Settings ==============
use_cross_validation = True  # Set to True to run 10-fold CV on training set
num_folds = 10
use_best_epochs = False
pause_encoding = True
transcript_type = "manual" # ASR_both_speakers, ASR_participant_only, manual, ASR_both_speakers_PE_medium
fine_tuned = False
prompt_type = "after" # both_original or both_v1 or different_BERT-V1_RoBERTa-Original, both_v1
model_used = "ensemble" # ensemble, bert or roberta
plm_size = "base"
num_epochs = 20
top_n_epochs = 3
majority_voting = True # True = hard majority voting, False = weighted majority voting
...
def run_pbft(seed, train_df, val_df, test_df):
    ...
    elif prompt_type == "after":
        template_bert = ['{"placeholder":"text_a"}. The diagnosis result is {"mask"}.']
        template_roberta = ['{"placeholder":"text_a"}. The diagnosis result is {"mask"}.']
    elif prompt_type == "before":
        template_bert = ['The diagnosis result is {"mask"}. {"placeholder":"text_a"}.']
        template_roberta = ['The diagnosis result is {"mask"}. {"placeholder":"text_a"}.']
    else:
        print('ERROR template never assigned')
    ...
    for plm_type, model_path in model_configs:
        ...
        for template_text in model_templates:
            ...
            epoch_logits, epoch_val_acc, epoch_models = [], [], []
            for epoch in tqdm(range(num_epochs), desc=f"{model_path}"):
                ...
                epoch_models.append(copy.deepcopy(model.plm))
            ...
            if use_best_epochs:
                ...
                top_epochs = [(acc, len(epoch_val_acc) - i - 1) for i, (acc, _) in enumerate(reversed(epoch_val_acc[-top_n_epochs:]))]

            for _, best_epoch in top_epochs:
                logits_stack = epoch_logits[best_epoch]
                acc_single = accuracy_score(all_labels_final, np.argmax(logits_stack, axis=1))
                ...
                # Save best models
                # save_dir = f"manual_transcript_expert/{plm_type}_{model_path.split('/')[-1]}_seed{seed}_epoch{best_epoch}"
                # os.makedirs(save_dir, exist_ok=True)
                # epoch_models[best_epoch].save_pretrained(save_dir)
                # tokenizer.save_pretrained(save_dir)
                
                # # Save validation accuracy
                # metrics_path = os.path.join(save_dir, "metrics.json")
                # with open(metrics_path, "w") as f:
                #     json.dump({"val_acc": float(acc_single)}, f)
...
def main():
    ...
    # GMM is fit once using full training set (can change later if you want CV-specific ones)
    ...
    #     for fold, (train_idx, val_idx) in enumerate(skf.split(train_df_full, train_df_full["label"])):
    #         print(f"\n=== Fold {fold+1} ===")
    #         train_df = train_df_full.iloc[train_idx].reset_index(drop=True)
    #         val_df = train_df_full.iloc[val_idx].reset_index(drop=True)
    #         acc = run_pbft(seed=42 + fold, train_df=train_df, val_df=val_df, test_df=test_df)
    #         accuracies.append(acc)
    
    if use_cross_validation:
        print(f"Running {num_folds}-fold cross-validation across 15 seeds...")
        seeds = range(42, 57)
        for seed in seeds:
            print(f"\n====== Seed {seed} ======")
        ...
        # Original method: fixed validation set per seed
        seeds = range(42, 43)
        for seed in seeds:
            print(f"\n=== Seed {seed} ===")
            train_df, val_df = train_test_split(train_df_full, test_size=0.2, stratify=train_df_full["label"], random_state=seed)
