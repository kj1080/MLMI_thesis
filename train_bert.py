...
# --- Configuration ---
if pause_encoding == True:
    CONTROL_TRANSCRIPT_DIR = "pause_encoding/train/cc"  
    CD_TRANSCRIPT_DIR = "pause_encoding/train/cd"
elif pause_encoding == False:
...
run = wandb.init(
    ...
    project="mlmi_thesis",
    name=f"bert_ad_text_run_{pd.Timestamp.now().strftime('%m%d_%H%M')}",
    config={
        "learning_rate": 1e-5,
...
if pause_encoding == True:
    ...
    CD_TRANSCRIPT_DIR = "pause_encoding/train/cd"
    rows = []
    for fname in os.listdir(CONTROL_TRANSCRIPT_DIR):
        if fname.endswith(".txt"):
            file_id = os.path.splitext(fname)[0].upper()
            label = 0
            with open(os.path.join(CONTROL_TRANSCRIPT_DIR, fname), "r") as f:
                text = f.read().strip()
            if text:
                rows.append({"text": text, "label": label})
    for fname in os.listdir(CD_TRANSCRIPT_DIR):
        if fname.endswith(".txt"):
            file_id = os.path.splitext(fname)[0].upper()
            label = 1
            with open(os.path.join(CD_TRANSCRIPT_DIR, fname), "r") as f:
                text = f.read().strip()
            if text:
                rows.append({"text": text, "label": label})
...
class SaveLogitsCallback(TrainerCallback):
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        os.makedirs("bert_results/checkpoint_logits", exist_ok=True)
        epoch_num = int(state.epoch)
        np.save(f"bert_results/checkpoint_logits/epoch_{epoch_num}_logits.npy", logits)
        np.save(f"bert_results/checkpoint_logits/labels.npy", labels)
...
training_args = TrainingArguments(
    ...
    save_strategy="epoch",
    save_total_limit=4,  # Keep last 3 + best
    ...
    logging_steps=5,
    report_to="wandb",
    save_steps=None,  # Ensure it saves only per epoch
)
