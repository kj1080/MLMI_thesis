# need to add saving mechansim to this as well - not quite finished!
...
def parse_args():
    ...
    parser.add_argument("--roberta_lora", action="store_true", default="True")
...
output_dir = "final_8th_august_ensemble_outputs/moe"  # You can change as needed
model_name = f"{model_used}_{plm_size}_{prompt_type}"  # Add more to the name if you want
...
def save_ensemble_outputs(
    ...
    val_weights,
    seed,
    output_dir="final_8th_august_ensemble_outputs/moe"
):
...
def run_pbft(seed, train_df, val_df, test_df):
    ...
    # --- Choose the right variables depending on if you're in span mode or not ---
    if span:
        output_span_ids = transcript_ids  # These are from your aggregation block
    ...
    # --- Save ensemble outputs just like non-MOE ---
