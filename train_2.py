...
fine_tuned = False
prompt_type = "both_original" # both_original or both_v1 or different_BERT-V1_RoBERTa-Original, both_v1, just_question_first_org
model_used = "ensemble"
plm_size = "base"
num_epochs = 20
top_n_epochs = 3
...
def run_pbft(seed, train_df, val_df, test_df):
    ...
    run = wandb.init(
        ...
        config={
            ...
            "epochs": num_epochs,
            "learning_rate": 4e-5, # 4e-5 if using LORA, 1e-5 if not
            "lora_r": 1,
            "lora_alpha": 512,
            "lora_dropout": 0.01,
            "num_experts": 4,
    ...
    token_label_counts = defaultdict(lambda: {0: np.zeros(num_experts), 1: np.zeros(num_experts)})  # If you want by diagnosis
        ...
        # Build model_name in the same style as non-moe
    model_name = f"moe_{plm_size}_{prompt_type}"
    ...
    # Save all raw logits and probs if needed
    all_raw_logits_per_model = np.array(all_preds_models) # [num_models, num_samples, num_classes]
    all_probs_per_model = np.array([torch.softmax(torch.tensor(x), dim=-1).numpy() for x in all_preds_models])
    np.save(os.path.join("ensemble_outputs/moe", f"all_logits_{model_name}_seed{seed}.npy"), all_raw_logits_per_model)
    np.save(os.path.join("ensemble_outputs/moe", f"all_probs_{model_name}_seed{seed}.npy"), all_probs_per_model)
    ...
    def save_ensemble_outputs(
        ...
        output_dir="ensemble_outputs/moe"   # where to save
    ):
    ...
    # Decide what IDs and labels to save (match non-moe logic)
    if span:
    ...
    meta = {
        ...
        "seed": seed,
        "moe": True,
        "num_experts": wandb.config.num_experts if hasattr(wandb.config, "num_experts") else None,
        "all_logits_file": f"all_logits_{model_name}_seed{seed}.npy",  # If you save these
        "all_probs_file": f"all_probs_{model_name}_seed{seed}.npy",
        # Add additional info if you want!
    }
    with open(os.path.join("ensemble_outputs/moe", f"meta_{model_name}_seed{seed}.json"), "w") as f:
